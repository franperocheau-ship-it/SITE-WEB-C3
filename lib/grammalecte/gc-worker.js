...
function init() {
  try {
    ...
    // Plusieurs groupes d'options utiles à nos 7 critères Code Champion
    // sont désactivés par défaut dans Grammalecte (heuristiques jugées à
    // risque de faux positifs pour un usage généraliste). On les active
    // explicitement ici — audit détaillé et justification par option dans
    // js/jogging-grammalecte.js (en-tête du fichier).
    gc_engine.setOption("poncfin", true); // P — ponctuation finale de paragraphe
    gc_engine.setOption("esp", true);     // P — espaces surnuméraires
    gc_engine.setOption("tab", true);     // P — tabulations surnuméraires
    gc_engine.setOption("nbsp", true);    // P — espaces insécables avant ! ? : ;
    gc_engine.setOption("unit", true);    // P — espaces insécables avant unités
    gc_engine.setOption("mapos", true);   // O — apostrophe manquante après l/d/s/n/c/j/m/t/ç
    gc_engine.setOption("mc", true);      // O — mots composés à trait d'union
...
function analyze(sText) {
  ...
  try {
    ...
    // Un saut de ligne dans un texte d'élève n'est jamais un nouveau
    // "paragraphe" isolé au sens de Grammalecte : c'est la continuation du
    // même texte. Sans cette normalisation, chaque \n coupe la règle
    // "majuscule après un point" (qui attend littéralement ". ", espace
    // compris) et fait retomber la ligne suivante dans le cas "paragraphe
    // d'une seule phrase" que Grammalecte ignore volontairement (anti-faux-
    // positifs sur des titres — cf. audit du 2026-07-20). Remplacement par
    // un espace (même longueur qu'un \n) : les positions de surlignage
    // renvoyées restent valables telles quelles sur le texte original.
    const sForAnalysis = sClean.replace(/\n/g, " ");
    const aGrammErr = gc_engine.parse(sForAnalysis, "FR", false, null, false);

    const oSpellChecker = gc_engine.getSpellChecker();
    const aSpellTokens = oSpellChecker.parseParagraph(sForAnalysis);
